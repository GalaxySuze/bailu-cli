...
> **v2.2.0+** · AI 工具侧的 Slash 命令，用于扫描项目并生成符合**白鹿规则规范**的项目规则文件。
...
| `/bailu-project-config` | Claude Code / Qoder | 扫描项目结构、识别技术栈、生成符合白鹿规则规范的规则**内容** |
...
更新 `CLAUDE.md`（如存在），添加规则索引和白鹿规则规范说明。
...
直接在 `.claude/rules/` 或 `.qoder/rules/` 下创建 `.md` 文件即可，遵循白鹿规则规范。`/bailu-project-config` 下次运行时会识别它们，提供“跳过/整理/重新生成”选项，不会粗暴覆盖。
