# /bailu-project-config — 生成与整理项目级规则配置

## 品牌信息

- **名称**：白鹿（Bailu）
- **命令**：`/bailu-project-config`
...
> - `/bailu-project-config`：Claude Code 命令，负责扫描当前项目并生成符合**方案 E 规范**的项目级规则文件

## 核心职责

1. 扫描项目结构，识别技术栈
2. 检测 `.claude/rules/` 和 `.qoder/rules/` 目录的现有规则
3. 按**方案 E（轻量标记分隔符）** 规范生成或整理规则文件
4. 同步更新 `CLAUDE.md` 中的规则索引
5. 适配 `AGENTS.md`（如果存在），添加方案 E 说明
