/**
 * HanaAgent 安装器（原 Hanako）
 *
 * 负责将白鹿工作流配置安装到 HanaAgent 的配置目录
...
class HanaAgentInstaller extends BaseInstaller {
  constructor() {
    const toolConfig = TOOLS['hana-agent'];
  ...
  async installRules(sourceDir, rules = []) {
    ...
    for (const rule of rules) {
      ...
      if (await fs.pathExists(rulePath)) {
        ...
        // HanaAgent 使用 write-file 模式，覆盖写入
  ...
  async uninstallRules(rules = []) {
    if (await fs.pathExists(this.rulesPath)) {
      // HanaAgent 使用 write-file 模式，卸载时删除文件
...
module.exports = HanaAgentInstaller;
