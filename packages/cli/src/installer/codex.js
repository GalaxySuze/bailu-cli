...
class CodexInstaller extends BaseInstaller {
  constructor() {
    const toolConfig = TOOLS.codex;
    ...
    // Codex 特有的文件路径
    this.instructionsPath = path.join(homeDir, 'instructions.md');
    this.configPath = path.join(homeDir, 'config.json');
  }

  /**
   * 安装 Rules 到 instructions.md
  ...
  async installRules(sourceDir, rules = []) {
    ...
    for (const rule of rules) {
      ...
      if (await fs.pathExists(rulePath)) {
        ...
        // Codex 使用 write-file 模式，覆盖写入
        await this.ensureDir(path.dirname(this.instructionsPath));
        await fs.writeFile(this.instructionsPath, content);
        installed.push(rule);
      }
    }
  ...
  /**
   * 卸载 Rules（清空 instructions.md）
   * @param {Array<string>} rules - 规则列表
   * @returns {Promise<Array<string>>} 已卸载的规则列表
   */
  async uninstallRules(rules = []) {
    if (await fs.pathExists(this.instructionsPath)) {
      // Codex 使用 write-file 模式，卸载时删除文件
      await fs.remove(this.instructionsPath);
      return rules;
    }
    return [];
  }
